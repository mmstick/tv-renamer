...
pub fn get_video_extensions() -> Result<Vec<String>, MimeError> {
    // Attempt to open the `/etc/mime.types` file.
    let mut file = fs::File::open("/etc/mime.types").map_err(|_| MimeError::OpenFile)?;
    ...
    file.read_to_string(&mut contents).map_err(|_| MimeError::ReadFileToString)?;
